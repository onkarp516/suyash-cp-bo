...
import { getMothertongue, createMothertongue } from "@/services/api_functions";
import {
  Tooltip,
  Modal,
  CloseButton,
...
// import Select from "react-select";
export default function MotherTongueSelectList(props) {
  ...
  const getMotherTongueOptions = (setVal = null) => {
    getMothertongue()
      .then((response) => {
        ...
        if (res.responseStatus == 200) {
          ...
          if (setVal != null && Opt.length > 0) {
            let mothertongue = getSelectValue(Opt, setVal);
            props.setFieldValue("motherTongueId", mothertongue);
          }
        }
      })
      .catch((error) => {
  ...
  const selecthandleChange = (newValue, actionMeta) => {
    props.setFieldValue("motherTongueId", newValue);
  };

  useEffect(() => {
    getMotherTongueOptions();
  }, []);

  // console.log("props==>", props);
  return (
    <div>
      <Form.Label htmlFor="motherTongueId" style={{ marginBottom: "0px" }}>
        Mother Tongue<span className="redstar">*</span>
        <a
          href=""
          onClick={(e) => {
            e.preventDefault();
      ...
      >
        ...
        <Formik
          ...
          validationSchema={Yup.object().shape({
            motherTongueName: Yup.string()
              .trim()
              .required("Mother Tongue name is required"),
          })}
          onSubmit={(values, { resetForm, setStatus, setSubmitting }) => {
            setStatus();

            let requestData = new FormData();
            requestData.append("motherTongueName", values.motherTongueName);

            createMothertongue(requestData)
              .then((response) => {
                if (response.data.responseStatus === 200) {
                  ...
                  setisLoading(false);
                  getMotherTongueOptions(response.data.response);
                  // setFieldValue();
                } else {
                  setSubmitting(false);
                }
                  ...
                  <Col md="4">
                    <Form.Group>
                      ...
                      <Form.Control
                        ...
                        value={values.motherTongueName}
                        isValid={
                          touched.motherTongueName && !errors.motherTongueName
                        }
                        styles={customStyles}
                        isInvalid={!!errors.motherTongueName}
                      />
