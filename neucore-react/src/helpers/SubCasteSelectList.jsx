...
export default function SubCasteSelectList(props) {
  const [isLoading, setisLoading] = useState(false);
  const [addModalShow, setAddModalShow] = useState(false);
  const [tooltipOpen, setTooltipOpen] = useState(false);
  const toggle = () => setTooltipOpen(!tooltipOpen);
  const [options, setoptions] = useState([]);
  const [opSubCasteList, setopSubCasteList] = useState([]);

  const [opReligionList, setopReligionList] = useState([]);
  const [opCasteList, setopCasteList] = useState([]);
  ...
  const getSubCasteData = (casteId, setVal = null) => {
      ...
      .then((response) => {
        ...
        if (res.responseStatus == 200) {
          ...
          // if (d.length > 0)
          {
            ...
            setoptions(Opt);
            if (setVal != null && Opt.length > 0) {
              let subCaste = getSelectValue(Opt, setVal);
              props.setFieldValue("subCasteId", subCaste);
            }
          }
        }
      })
  ...
  const onAddModalShow = (status) => {
    if (status == true) {
      let initVal = {
        ...
        casteId: props.casteId ? props.casteId : "",
      };
      setinitVal(initVal);
    }
  ...
  const selecthandleChange = (newValue, actionMeta) => {
    props.setFieldValue("subCasteId", newValue);
  };
  ...
  useEffect(() => {
    // getCasteOption();
    console.log("props.casteId ==================>>>>>>>>", props.casteId);
    getReligionData();
    if (
      props.casteId != "" &&
      props.casteId != null &&
      props.casteId != undefined
    ) {
      console.log("casteID props==---->>>>", props.casteId);
      //   getCasteData(props.religionId.value);
      getSubCasteData(props.casteId.value);
    } else {
      selecthandleChange("");
    }
  }, [props.casteId]);
  ...
  return (
    <div>
      <Form.Label htmlFor="subCasteId" style={{ marginBottom: "0px" }}>
        SubCaste <span className="redstar">*</span>
        <a
          href=""
          onClick={(e) => {
            e.preventDefault();
      ...
      >
        ...
        <Formik
          ...
          validationSchema={Yup.object().shape({
            ...
            casteId: Yup.object().required("Caste name is required"),
            subCasteName: Yup.string()
              .trim()
              .required("SubCaste name is required"),
          })}
          onSubmit={(values, { resetForm, setStatus, setSubmitting }) => {
            ...
            requestData.append("subCasteName", values.subCasteName);

            createSubCaste(requestData)
              .then((response) => {
                ...
                if (response.data.responseStatus === 200) {
                  ...
                  // setFieldValue();
                  getSubCasteData(values.casteId.value, response.data.response);
                } else {
                  setSubmitting(false);
                }
              })
            ...
            <Form autoComplete="off" onSubmit={handleSubmit}>
              <Modal.Body className="p-4 p-invoice-modal common-form-style">
                <Row>
                  ...
                  <Col md="3">
                    <Form.Group>
                      <Form.Label>SubCaste</Form.Label>
                      <Form.Control
                        type="text"
                        ...
                        value={values.subCasteName}
                        isValid={touched.subCasteName && !errors.subCasteName}
                        isInvalid={!!errors.subCasteName}
