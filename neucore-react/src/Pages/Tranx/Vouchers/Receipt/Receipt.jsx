...
export default class TranxPurchaseInvoiceCreate extends React.Component {
  ...
  getDebtorLedgersByBranchFun = (branch_id) => {
    let requestData = new FormData();
    requestData.append("branchId", branch_id);

    getDebtorLedgersByBranch(requestData)
      .then((response) => {
        console.log("response", response);
        let res = response.data ? response.data : [];
        let resLst = [];
        ...
        if (res.responseStatus == 200) {
          ...
          this.setState({ lstSundryDebtors: resLst });
        }
      })
      .catch((error) => {
        console.log("error", error);
          ...
          <Formik
            ...
            onSubmit={(values, { setSubmitting, resetForm }) => {
                ...
                if (
                  ...
                  let billRow =
                    ...
                    v.perticulars.billids.map((vi, ii) => {
                      if ("paid_amt" in vi && parseInt(vi["paid_amt"]) > 0) {
                        return vi;
                      }
                      if (
                        "credit_paid_amt" in vi &&
                        parseInt(vi["credit_paid_amt"]) > 0
                      ) {
                        // return vi;
                        return {
                  ...
                  return {
                    type: v.type,
                    paid_amt: v.type == "cr" ? v.paid_amt : v.credit,
              ...
              if (values.branchId != null && values.branchId != "")
                formData.append("underBranchId", values.branchId.value);

              if (
                values.studentLedgerId != null &&
                values.studentLedgerId != ""
              )
                formData.append(
                  "studentLedgerId",
                  values.studentLedgerId.value
                );
                  ...
                  <Row>
                    ...
                    <Col lg="2" md="2">
                      <Select
                        ...
                        onChange={(v) => {
                          ...
                          this.setState({ lstSundryDebtors: [] });
                          if (v != null) {
                            setFieldValue("branchId", v);
                            this.getDebtorLedgersByBranchFun(v.value);
                          }
                        }}
                        name="branchId"
                    ...
                    <Col lg="2" md="2">
                      <Select
                        ...
                        styles={customStyles}
                        onChange={(v) => {
                          setFieldValue("studentLedgerId", "");
                          if (v != null) {
                            setFieldValue("studentLedgerId", v);
                          }
                        }}
                        name="studentLedgerId"
                        options={lstSundryDebtors}
                        value={values.studentLedgerId}
                        invalid={errors.studentLedgerId ? true : false}
                      />
                    </Col>
                  </Row>
                </div>
