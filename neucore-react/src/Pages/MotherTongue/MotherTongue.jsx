...
import { customStyles, MyNotifications } from "@/helpers";
...
export default class MotherTongue extends React.Component {
  constructor(props) {
    super(props);
    this.motherTongueFormRef = React.createRef();
    this.state = {
      data: [],
  ...
  getmotherTonguebyidlst = (motherTongueId) => {
    let reqData = new FormData();
    reqData.append("id", motherTongueId);
    getMotherTongueById(reqData)
      .then((response) => {
        let res = response.data;
        if (res.responseStatus == 200) {
          // this.setState({ initVal: res.responseObject, opendiv: true });
          let result = res.responseObject;
          let initVal = {
            id: result.id,
            motherTongueName: result.name,
          };
        ...
        <Collapse in={opendiv}>
          <div id="example-collapse-text" className="common-form-style p-1">
            <div className="main-div mb-2 m-0">
              <h4 className="form-header">Mother Tongue</h4>
              <Formik
                validateOnChange={false}
                // validateOnBlur={false}
                enableReinitialize={true}
                initialValues={initVal}
                innerRef={this.motherTongueFormRef}
                ...
                onSubmit={(values, { resetForm, setStatus, setSubmitting }) => {
                  ...
                  if (values.id == "") {
                    ...
                    updateMotherTongue(requestData)
                      .then((response) => {
                        this.setState({ isLoading: false });
                        if (response.data.responseStatus === 200) {
                          setSubmitting(false);
                    ...
                    data.map((v, i) => {
                      ...
                      return (
                        <tr
                          ...
                          onDoubleClick={(e) => {
                            e.preventDefault();
                            this.getMothertongueData(v.id);
                          }}
                        >
                          <td>{i + 1}</td>

                          <td>{v.motherTongueName}</td>
                          ...
                          <td>
                            <a
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                this.getmotherTonguebyidlst(v.id);
                              }}
                            >
                              <img
                                src={edit_ico}
